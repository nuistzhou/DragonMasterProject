...
ndvi_annual <- function(ndvi_data, ndvi_reliability) {
  # Select good quality ndvi data based on the data realibility
  ndvi_stack <- stack()
  for (i in 1:12) {
    ndvi[[i]] <- overlay(x=ndvi[[i]],y=ndvi_reliability[[i]],fun=function(x,y) ifelse(y %in% c(0,2,4),NA,x))
    ndvi_stack <- stack(ndvi_stack, ndvi[[i]])
    }
  # Do mean calculation to get annual NDVI stack
  return (calc(ndvi_stack, fun = mean))
}
